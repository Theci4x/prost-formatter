...
function silence(data: PlatformReviews, label: string): string | null {
  if (!data.configured) {
  ...
  if (data.businessStatus === "CLOSED_TEMPORARILY") {
    ...
    // réouverture.
    return `Votre fiche ${label} est marquée « fermée temporairement ». Tant que c'est le cas, ${label} ne transmet plus vos avis — ils reviendront d'eux-mêmes à la réouverture.`;
  }
  if ((data.reviewCount ?? 0) > 0) {
    return `${label} affiche ${data.reviewCount} avis mais n'en transmet aucun pour le moment. Ils restent lisibles sur la fiche.`;
  }
  return `Aucun avis sur ${label} pour le moment.`;
}
...
export function TuilePlateforme({
  ...
  const message = silence(data, label);
          ...
          <span className="flex flex-wrap items-center gap-x-3 gap-y-1">
            ...
            {data.reviewCount != null && (
              <span className="text-sm text-zinc-600">
                {data.reviewCount.toLocaleString("fr-FR")} avis
              </span>
            )}
          </span>
        </div>
  ...
  return (
    ...
    >
      ...
      {avis.text ? (
        ...
        <p className="text-sm italic text-zinc-400">Note sans commentaire.</p>
      )}
